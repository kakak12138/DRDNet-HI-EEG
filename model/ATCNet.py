...
class CausalConv1d(nn.Conv1d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, dilation=1, **kwargs):
        # 计算需要填充的长度：(Kernel_Size - 1) * Dilation
    ...
    def forward(self, x):
        ...
        # F.pad 参数格式: (padding_left, padding_right, padding_top, padding_bottom, ...)
        # 这里只对最后一个维度 (Time) 的左侧进行填充
        if self.__padding != 0:
...
class _ConvBlock(nn.Module):
    def __init__(self, F1: int = 16, kernel_length: int = 64, pool_length: int = 8,
                 D: int = 2, in_channels: int = 32, dropout: float = 0.3):
