...
class TrainModel:
    ...
    def train_model(self, model_class, n_classes, dataset, learning_rate=0.001, batch_size=64, epochs=500, n_splits=5,
        ...
        for fold, (train_idx, val_idx) in enumerate(kf.split(dataset, y_labels_for_stratify)):
            ...
            for epoch in range(epochs):
                ...
                if val_accuracy > best_val_acc_for_print:
                    print(f"Epoch {epoch + 1}/{epochs} Train Loss: {epoch_loss:.4f} Val Loss: {val_loss:.4f} Val Acc: {val_accuracy * 100:.2f}%")
            ...
            print(f"Fold {fold + 1} Best Val Acc: {best_val_accuracy_in_fold * 100:.2f}%")
            print(f"Fold {fold + 1} Best Val Loss: {best_val_loss_in_fold:.4f}")
            if log_write:
        ...
        print(f"Best {selection_metric} at epoch {best_epoch}")
        if log_write:
        ...
        if selection_metric == 'loss':
            print(f"Best Val Loss: {overall_best_val_metric:.4f} Acc: {best_val_acc_at_best_loss * 100:.2f}%")
            if log_write:
                log_write.write(
                    f"Best Global Val Loss: {overall_best_val_metric:.4f} (Acc: {best_val_acc_at_best_loss * 100:.2f}%)\n")
        else:
            print(f"Best Val Acc: {overall_best_val_metric * 100:.2f}% Loss: {best_val_loss_at_best_acc:.4f}")
            if log_write:
                log_write.write(
